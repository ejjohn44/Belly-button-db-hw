...
function init() {
  ...
  d3.json("/names").then((sNames) => {
    ...
    const beginSample = sNames[0];
    buildCharts(beginSample);
    buildMetadata(beginSample);
  });
}

function optionChanged(addSample) {
  buildCharts(addSample);
  buildMetadata(addSample);
}
